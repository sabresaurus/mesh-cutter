...
public class TempMesh
{
    ...
    public TempMesh(int vertexCapacity)
    {
        ...
        boneWeights = new List<BoneWeight>(vertexCapacity);
    ...
    /// </summary>
    private void AddPoint(Vector3 point, Vector3 normal, Vector2 uv, BoneWeight boneWeight)
    {
    ...
    public void AddSlicedTriangle(int i1, 
                                  Vector3 v2, Vector2 uv2, BoneWeight bw2, 
                                  int i3)
    {
        int v1 = vMapping[i1],
            v3 = vMapping[i3];
    ...
    /// </summary>
    public void AddTriangle(Vector3[] points, BoneWeight boneWeight)
    {
        // Compute normal
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            // TODO: Use per vertex bone weights
            AddPoint(points[i], normal, Vector2.zero, boneWeight);
        }
    ...
    /// </summary>
    public void AddVertex(List<Vector3> ogVertices, List<Vector3> ogNormals, List<Vector2> ogUvs, List<BoneWeight> ogBoneWeights, int index)
    {
        vMapping[index] = vertices.Count;
        vertices.Add(ogVertices[index]);
        normals.Add(ogNormals[index]);
        uvs.Add(ogUvs[index]);
        boneWeights.Add(ogBoneWeights[index]);
    }
